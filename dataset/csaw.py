...
class CSAWDataset(Dataset):
    def __init__(self, data_dir, healthy=True, model_dim=256, transform=transforms.Lambda(lambda x: x)):
        self.data_dir = data_dir
        self.transform = transform
        self.healthy = healthy
        self.model_dim = model_dim
        self.images = [entity.path for entity in os.scandir(data_dir) if entity.name.endswith('.dcm')]
    ...
    def __getitem__(self, idx):
        ...
        if self.healthy:
            ...
            mask_path = image_path.replace('.dcm', '_mask.png').replace('images', 'masks')
